...
const SubProducts = ({ productId, vendorId }: { productId: number, vendorId: number }) => {
    ...
    const dataTableColumns: SearchableColumnDef<SubProduct>[] = [
        ...
        },
        {
            ...
            header: 'Expiry Date',
            searchable: false,
            cell: (info) => info.row.original.expiry_date ? format(new Date(info.row.original.expiry_date), 'd MMM, yyyy') : info.row.original.expiry_date
        },
        {
            id: 'status',
            accessorKey: 'expiry_date',
            header: 'Status',
            cell: (info) => {
                const expiryDate = info.getValue() as string | null;
        ...
        },
        {
            ...
            cell: (info) => (
                <ActionDropdown>
                    ...
                    <li>
                        <a onClick={() => toggleBatchStatus({ vendorId, productId, subProductId: info.row.original.id as number })}>
                            <Rss className="w-4 h-4" />
                            {info.row.original.status ? 'Unpublish' : 'Publish'}
                        </a>
                    </li>
