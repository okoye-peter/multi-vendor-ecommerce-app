...
const Index = () => {
    ...
    //         header: 'Unit Price',
    //         cell: ({ getValue }: { getValue: () => number }) => `₦${(getValue() as number).toFixed(2)}`,
    //     },
    //     {
    //         id: 'total',
    //         // accessorKey: undefined, // Since we're computing this value
    //         header: 'Total Price',
    //         cell: ({ row }: { row: { original: Order } }) => `₦${(row.original.quantity * row.original.priceOnPurchase).toFixed(2)}`,
    //     },
    //     {
    //         id: 'created_at',
    //         accessorKey: 'createdAt',
    //         header: 'Created At',
    //         cell: ({ getValue }: { getValue: () => string }) => format(new Date(getValue() as string), 'd MMM, yyyy'),
    //     },
    // ];
    ...
    const dataTableColumns: SearchableColumnDef<Order>[] = [
        ...
        },
        {
            id: 'total',
            header: 'Total Price',
            cell: (info) => `₦${(info.row.original.quantity * info.row.original.priceOnPurchase).toFixed(2)}`,
        },
        {
            id: 'created_at',
            accessorKey: 'createdAt',
