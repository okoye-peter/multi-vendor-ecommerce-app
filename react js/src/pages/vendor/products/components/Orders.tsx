...
const Orders = ({ productId, vendorId }: { productId: number, vendorId: number }) => {
    ...
    const dataTableColumns: SearchableColumnDef<Order>[] = [
        ...
        },
        {
            ...
            header: 'Batches',
            cell: ({ getValue }) => {
                const orderSubProducts = getValue() as orderSubProduct[];
                ...
                return <span className='flex flex-col gap-2 '>
                    {orderSubProducts.map(orderSubProduct => (<span className="px-3 py-1 text-xs text-blue-900 bg-gray-100 rounded w-fit">{orderSubProduct.subProduct?.batch_no} - {orderSubProduct.quantity} unit(s)</span>))}
                </span>
            },
        },
